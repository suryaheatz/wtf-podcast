...
export const GuestAvatarStack = ({
  guests,
  maxVisibleMobile = 2,
}: GuestAvatarStackProps): JSX.Element => {
      ...
      <div className="hidden md:flex md:flex-wrap md:gap-3">
        {guests.map((guest, index) => (
          <div
            key={index}
            className="
              flex items-center gap-3 pl-2 pr-5 py-2.5 rounded-full border transition-all duration-200
              bg-white border-gray-200 hover:bg-gray-50
              dark:bg-zinc-900 dark:border-[#fffefe0d] dark:hover:bg-zinc-800
            "
          >
            ...
            <div className="flex flex-col leading-tight">
              <span className="text-[15px] font-medium [font-family:'Arial-Regular',Helvetica] text-gray-900 dark:text-white">
                {guest.name}
              </span>
              <span className="text-xs [font-family:'Arial-Regular',Helvetica] mt-0.5 text-gray-600 dark:text-[#70707b]">
                {guest.title}
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {visibleGuestsMobile.map((guest, index) => (
            <div
              key={index}
              className="
                flex items-center gap-2.5 pl-1.5 pr-4 py-1.5 rounded-full border transition-all duration-300 ease-out
                bg-white border-gray-200 hover:bg-gray-50
                dark:bg-zinc-900 dark:border-[#fffefe0d] dark:hover:bg-zinc-800
            ...
            >
              ...
              <div className="flex flex-col leading-tight">
                <span className="text-sm font-medium [font-family:'Arial-Regular',Helvetica] text-gray-900 dark:text-white">
                  {guest.name}
                </span>
                <span className="text-[11px] [font-family:'Arial-Regular',Helvetica] mt-0.5 text-gray-600 dark:text-[#70707b]">
                  {guest.title}
                </span>
              </div>
            </div>
